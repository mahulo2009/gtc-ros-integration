...
// Includes
#include "useFilterWheel.h"

int main(int argc, char** argv)
{
	CommandLine::current(argc, argv);
	CommandLine cl(argc, argv);
	char *devicename = cl.getOption("-name","Test/FilterWheel_1");
	CORBA::Object* obj = 0;
	FILTERWHEEL::FilterWheel_ifce* device = 0;
	int quit=FALSE;
	int option, result=0;
	...
	try
	{
		...
		try
		{
			device = FILTERWHEEL::FilterWheel_ifce::_narrow(obj);
		}
		...
		{
			cerr << "Reference is not a FILTERWHEEL::FilterWheel_ifce" << endl;
			throw 0;
		}
		printf(" - ping FilterWheel\n");
		device->ping();
		printf(" - device found\n");
