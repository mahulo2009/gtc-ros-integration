...
// Includes
#include "useIcaroDas.h"

#ifdef VXWORKS
int useIcaroDas(char *devicename)
{
	if(devicename==NULL)
	{
		cerr << "Usage:  useIcaroDas(devicename)" << endl;
		return(1);
	} // Default
#else
int main(int argc, char** argv)
{
	...
	char *devicename = cl.getOption("-name","Test/IcaroDas_1");
#endif
	CORBA::Object* obj = 0;
	ICARODAS::IcaroDas_ifce* device = 0;
	int quit=FALSE;
	int option, result=0;
	...
	try
	{
		...
		try
		{
			device = ICARODAS::IcaroDas_ifce::_narrow(obj);
		}
		...
		{
			cerr << "Reference is not a ICARODAS::IcaroDas_ifce" << endl;
			throw 0;
		}
		printf(" - ping IcaroDas\n");
		device->ping();
		printf(" - device found\n");
